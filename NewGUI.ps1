...
# New-PSHost #
##############
# Background host functions - Working progress...
function New-PSHost {
    param (
        [String[]]$Module
    )
...
# Invoke-GuiCommand #
#####################
# This function handles how the GUI 
...
# Xaml code of how the gui will look.
...
# What happens when certain parameters have been selected and is executed (run)
$MainWindow.FindName("Run").Add_Click( {
    ...
    # but we don't touch them beyond that
        ...
        if ($shouldInvoke) {
            # If you select 'Yes' to the confirmation message then the command will be invoked
            try {
